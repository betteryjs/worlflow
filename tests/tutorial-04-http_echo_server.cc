...
void process(WFHttpTask *server_task)
{
	...
	len = snprintf(buf, BUFFERSIZE, "<p>%s %s %s</p>", req->get_method(),
				   req->get_request_uri(), req->get_http_version());
	resp->append_output_body(buf, len);

	while (cursor.next(name, value))
	{
		len = snprintf(buf, BUFFERSIZE, "<p>%s: %s</p>", name.c_str(), value.c_str());
		resp->append_output_body(buf, len);
	}
	...
	if (addr.ss_family == AF_INET)
	{
		...
		inet_ntop(AF_INET, &sin->sin_addr, addrstr, IPSIZE);
	...
	else if (addr.ss_family == AF_INET6)
	{
		...
		inet_ntop(AF_INET6, &sin6->sin6_addr, addrstr, IPSIZE);
